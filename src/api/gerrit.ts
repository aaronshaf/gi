    ...
    Effect.gen(function* () {
      ...
      const getMessages = (changeId: string) =>
        Effect.gen(function* () {
          ...
          // Extract messages from the change response
          return (response as { messages?: MessageInfo[] }).messages || []
        }).pipe(
          Effect.map((messages) =>
            // Filter out automated messages and empty messages, keep meaningful review comments
            messages.filter((msg) => {
